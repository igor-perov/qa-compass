...
PDF export is currently treated as experimental and is not generated by default. HTML reports are the canonical output because they render more predictably.
...
The packaged PDF export helper remains available for explicit experimental exports and supports an `npx` fallback when a global `playwright-cli` is not installed.
