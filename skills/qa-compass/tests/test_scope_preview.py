...
class ScopePreviewTests(unittest.TestCase):
    ...
    def test_builds_pre_execution_scope_preview_grouped_by_feature(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(preview["execution_readiness"]["confirmation_required"])
            self.assertTrue(preview["execution_readiness"]["must_stop_before_execution"])
            self.assertTrue(preview["execution_readiness"]["requires_otp"])
            self.assertIn("Provide the exact environment URL to test.", preview["execution_readiness"]["blocking_questions"])

            html = (tmp_root / "preview" / "qa-scope-preview.html").read_text(encoding="utf-8")
            self.assertIn("QA Scope Preview", html)
            self.assertIn("Confirmation Required", html)
            self.assertIn("Do not start browser execution", html)
            self.assertIn("OTP / MFA", html)
            ...
            self.assertIn("## Confirmation Gate", markdown)
            self.assertIn("OTP/MFA handling needed: `yes`", markdown)
